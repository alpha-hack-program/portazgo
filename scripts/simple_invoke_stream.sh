#!/usr/bin/env bash
# Get APPS_DOMAIN from OpenShift (oc), set LLAMA_STACK_HOST, then run the simple_invoke_stream example (streaming output).
# Arguments = the question to ask the agent (not the script path).
#
# Usage (from portazgo):
#   ./scripts/simple_invoke_stream.sh "What is 2+2?"
#   ./scripts/simple_invoke_stream.sh "Tell me about taxes in Lysmark."
#   ./scripts/simple_invoke_stream.sh                    # uses default question
#
...
# Strip any leading arg that looks like the example script path (e.g. user pasted "examples/simple_invoke_stream.py" by mistake)
INVOKE_ARGS=()
for arg in "$@"; do
...
echo "Running: uv run python examples/simple_invoke_stream.py ${INVOKE_ARGS[*]}"
echo "---"
exec uv run python examples/simple_invoke_stream.py "${INVOKE_ARGS[@]}"
